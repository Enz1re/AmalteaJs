...
function Model(modelObj, viewNode, presenter) {
    ...
    this._inner = {};
    var self = this;
    Object.getOwnPropertyNames(modelObj.model).forEach(function (prop) {
        if (typeof modelObj.model[prop] === 'object') {
            self._inner[prop] = modelObj.model[prop];
            if (!self._inner[prop]) {
                self._inner[prop].value = null;
            }
        } else {
            self._inner[prop] = { type: modelObj.model[prop], value: null };
        }
    ...
    this.onValueChanged = function (callback, throwException) {
        ...
        viewNode.addEventListener(propChangeEventName, function (e) {
            ...
            if (value) {
                ...
                if (throwException) {
                    throw new Error("Error validating model " + e.detail.prop + " " + e.detail.value);
                }
            }
        });
    }
